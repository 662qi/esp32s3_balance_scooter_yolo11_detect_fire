...
// 队列元素：存放去掉首尾括号的有效内容，最大长度 RX_PACKET_MAX_LEN
QueueHandle_t cmd_queue = NULL;
#define CMD_QUEUE_LENGTH 20

// 外部引用蓝牙解析任务句柄，用于收到数据包唤醒解析任务
extern TaskHandle_t ble_parse_task_handle;
...
static int ble_svc_gatt_handler(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR: {
        ...
        if (len <= 2 || len > RX_PACKET_MAX_LEN)  // 修正长度判断
            break;

        uint8_t buf_temp[RX_PACKET_MAX_LEN] = {0};
        os_mbuf_copydata(ctxt->om, 0, len, buf_temp);

        // 必须是 [ 开头 ] 结尾
        if (buf_temp[0] == '[' && buf_temp[len - 1] == ']') {
            char cmd[RX_PACKET_MAX_LEN] = {0};
            memcpy(cmd, buf_temp + 1, len - 2);
            // 发送到队列，避免丢失数据。小等待时间避免在队列满时长期阻塞 BLE 协议栈。
            if (xQueueSend(cmd_queue, cmd, pdMS_TO_TICKS(10)) != pdPASS) {
                MODLOG_DFLT(ERROR, "cmd_queue full, drop BLE cmd: %s\n", cmd);
            } else {
                MODLOG_DFLT(INFO, "BLE enqueue: %s", cmd);
            }
        }
        break;
    }
...
void ble_server_uart_task(void *pvParameters)
{
    ...
    static uint8_t p_rx = 0;
    static char uart_rx_buf[RX_PACKET_MAX_LEN] = {0};  // 局部静态缓冲

    for (;;) {
        if (xQueueReceive(spp_common_uart_queue, &event, portMAX_DELAY)) {
            if (event.type == UART_DATA && event.size > 0) {
                int len = uart_read_bytes(UART_NUM_1, temp_buf, event.size, portMAX_DELAY);
                for (int i = 0; i < len; i++) {
                    ...
                    if (rx_state == 0) {
                        ...
                        if (ch == ']') {
                            uart_rx_buf[p_rx] = '\0';
                            if (xQueueSend(cmd_queue, uart_rx_buf, pdMS_TO_TICKS(10)) != pdPASS) {
                                MODLOG_DFLT(ERROR, "cmd_queue full, drop UART cmd: %s\n", uart_rx_buf);
                            }
                            rx_state = 0;
                        } else if (p_rx < RX_PACKET_MAX_LEN - 1) {
                            uart_rx_buf[p_rx++] = ch;
                        }
                    }
                }
...
static void ble_spp_uart_init(void)
{
    ...
    // 重点：指定UART1 + 定义的GPIO36 TX、GPIO37 RX
    uart_param_config(UART_NUM_1, &uart_config);

    // TX=36  RX=37  流控引脚不用填-1
    ...
    // 缓冲区大小对齐你的配置
    uart_driver_install(UART_NUM_1, 1024, 1024, 10, &spp_common_uart_queue, 0);

    xTaskCreate(ble_server_uart_task, "uTask", 4096, (void *)UART_NUM_1, 8, NULL);
}

// 新增：串口发送字符串底层函数
static void ble_uart_send_string(const char *str)
{
    if (str == NULL) return;
...
myble_init(void)
{
    ...
    cmd_queue = xQueueCreate(CMD_QUEUE_LENGTH, RX_PACKET_MAX_LEN);  // 缓存命令
    ...
    rc = ble_svc_gap_device_name_set("esp_balance");
    assert(rc == 0);
#endif
